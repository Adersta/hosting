...
/*
    ...
    pause_label = game.add.text(400, 400, 'Pause');
	pause_label.anchor.setTo(0.5);
		
	pause_label.font = 'Revalia';
	pause_label.fontSize = 50;
    ...
    function unpause(event){
        ...
        if(game.paused){
			...
			//HUD
		text = game.add.text(96, 536,"asdasjkdbajskd bkjasdbasdkasdjsabdsalbdjasbdasbdlsajdsadasjdkbsd");
...
IA:function (localenemy) {			
			
	var dx = player.x - localenemy.x;
	var dy = player.y - localenemy.y;
	distanceEnemy = Math.sqrt(dx * dx + dy * dy);
...
quest:function(){
	...
	let RanQuest = Math.floor(Math.random() * 7);
	...
	// And a label to illustrate which menu item was chosen. (This is not necessary)
	choiseLabel = game.add.text(w/2, h-150, ''+preguntas[RanQuest], { font: '30px Arial', fill: '#fff' });
	choiseLabel.anchor.setTo(0.5, 0.5);
	choiseLabel.fixedToCamera = true;
