...
create:function () 
{
	...
	let randompos = Math.floor(Math.random() * ((w/2 +100) - (w/2 -150)) + (w/2 -150)); 
...
update:function () 
{
	...
	if(video.currentTime>=7.8 && FinalGameFase==true){
		video.paused = true;
	}
	...
	if(checkOverlap(puntero, sprite2) && fireButton.isDown && ApuntadoGamefase == true)
    {
        posibility = posibility + 10;
		if(posibility > 10){
			posibility = 10;

		}
    }
    else if(ApuntadoGamefase == true)
    {
		posibility = posibility - 10;
		if(posibility < 10){
			posibility = 0;
